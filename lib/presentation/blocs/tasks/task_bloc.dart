...
class TaskBloc extends Bloc<TaskEvent, TaskState> {
  ...
  Future<void> _onLoadAllTasks(
    ...
    print('TaskBloc: Loading all tasks...');
    ...
    result.fold(
      (failure) {
        print('TaskBloc: Error loading tasks - ${failure.message}');
  ...
  Future<void> _onLoadTasksForManager(
    ...
    print('TaskBloc: Loading tasks for manager (category-filtered)...');
    emit(const TaskLoading());
    ...
    result.fold(
      (failure) {
        print('TaskBloc: Error loading tasks for manager - ${failure.message}');
        emit(TaskError(message: failure.message));
      },
      (tasks) {
        print('TaskBloc: Loaded ${tasks.length} tasks for manager');
        emit(
          TasksLoaded(
            tasks: tasks,
            hasMore: tasks.length == (event.limit ?? 20),
  ...
  Future<void> _onLoadTaskById(
    ...
    print('TaskBloc: Loading task by ID: ${event.taskId}');
    emit(const TaskLoading());
    ...
    result.fold(
      (failure) {
        print('TaskBloc: Error loading task by ID - ${failure.message}');
        emit(TaskError(message: failure.message));
      },
      (task) {
        print('TaskBloc: Loaded task by ID: ${task.title}');
    ...
    await result.fold(
      ...
      (task) async {
        ...
        try {
          ...
          print('🔔 TaskBloc: Task title: ${task.title}');
          print('🔔 TaskBloc: Employee: ${task.employeeName}');
          ...
          // Log notification failure but don't fail the task creation
          print('🔔 TaskBloc: ❌ Exception sending notifications: $e');
        }
      },
    );
  }
  ...
  Future<void> _onDeleteTask(DeleteTask event, Emitter<TaskState> emit) async {
    ...
    result.fold(
      (failure) {
        print('🗑️ TaskBloc: Delete failed: ${failure.message}');
        emit(TaskError(message: failure.message));
      },
      (_) {
        print('🗑️ TaskBloc: Delete successful for ${event.taskId}');
        emit(
          TaskDeleted(
            taskId: event.taskId,
  ...
  Future<void> _onAutoCleanupCompletedTasks(
    ...
    print('🧹 TaskBloc: Starting auto-cleanup of completed tasks...');
    emit(const TaskLoading());
    ...
    result.fold(
      (failure) {
        print('🧹 TaskBloc: Auto-cleanup failed: ${failure.message}');
        emit(TaskError(message: failure.message));
      },
      (cleanupResult) {
        print(
